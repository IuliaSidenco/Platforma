...
let currentTasks = []; //массив заданий, который сейчас решают
let currentIndex = 0; 
...
function openTasks(type, subject) {
  ...
  // Создаём переменную для заголовка, пока пустую
let title = "";

// Определяем, какой тип выбран: тест или упражнение
if (type === "tests") {
  title = "Тесты — "; // Если это тест, первая часть заголовка "Тесты — "
} else {
  title = "Упражнения — "; // Если это упражнение, первая часть заголовка "Упражнения — "
}

// Определяем тему (subject) и добавляем её к заголовку
if (subject === "algebra") {
  title += "Алгебра"; // Если алгебра, добавляем "Алгебра"
} else if (subject === "geometry") {
  title += "Геометрия"; // Если геометрия, добавляем "Геометрия"
} else {
  title += "Тригонометрия"; // Иначе добавляем "Тригонометрия"
}

// Находим HTML-элемент с id="tasksTitle" и вставляем туда собранный заголовок
document.getElementById("tasksTitle").textContent = title;
